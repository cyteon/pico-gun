--[[pod_format="raw",created="2025-07-30 19:56:58",modified="2025-07-30 20:05:50",revision=17]]
--[[

	i copied this fucking 1600+ line file just to change one fucking line
	cause cant modify system files permanently
...
function attach_text_editor_fixed(g, parent)
...
--	local function hydrate(line, token_state)
	...
	local function show_cursor(visible_x, visible_y)
		...
		content.x = mid(
		  -(xx - visible_x),
		  content.x,
		  -- this was the only line changed:
		  -(xx - (container.width - content.margin_left1 - char_w * 2))
		)
...
--[[
	...
	local function backup_line_edit()
		...
		if (cur_y ~= last_line_y or ((sub(text[cur_y],cur_x-1,cur_x-1) == " ") != (sub(text[cur_y],cur_x,cur_x) == " "))) then
			checkpoint()
			if (cur_y ~= last_line_y) highlight_y = nil -- nice place to invalidate -- highlight disappears once start editing
		end
		last_line_y = cur_y
	end
