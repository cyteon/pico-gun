...
function _update()
	if btnp(0) and not collides_left(p.x, p.y) then -- left
		p.dir = 0
		p.sprite = 2
		p.h_flip = true
	elseif btnp(1) and not collides_right(p.x, p.y) then -- right 
		p.dir = 1
		p.sprite = 2 
		p.h_flip = false
	elseif btnp(2) and not collides_up(p.x, p.y) then -- up 
		p.dir = 2
		p.sprite = 3
		p.v_flip = false
	elseif btnp(3) and not collides_down(p.x, p.y) then -- down 
		p.dir = 3
		p.sprite = 3
		p.v_flip = true
	end
	...
	if p.dir == 0 then
		...
		if not collides_left(p.x, p.y) then
			p.x -= 1
		end
	elseif p.dir == 1 then
		if p.y > ty then p.y -= min(1, p.y - ty)
		...
		if not collides_right(p.x, p.y) then
			p.x += 1
		end
	elseif p.dir == 2 then
		...
		if not collides_up(p.x, p.y) then
			p.y -= 1
		end
	elseif p.dir == 3 then
		if p.x > tx then p.x -= min(1, p.x - tx)
		...
		if not collides_down(p.x, p.y) then
			p.y += 1
		end
	end
